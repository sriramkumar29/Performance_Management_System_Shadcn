...
// Dynamically loaded API module (so env stubs apply before import)
let apiFetch: typeof import("./api")["apiFetch"];
let api: typeof import("./api")["api"];

// Ensure env vars are stubbed before importing the module under test
beforeEach(async () => {
  vi.restoreAllMocks();
  vi.resetModules();
  vi.unstubAllEnvs();
  ...
  // Clear sessionStorage and reset fetch mock per test
  sessionStorage.clear();
  global.fetch = vi.fn();

  // Spy storage setItem for tests that assert calls
  vi.spyOn(Storage.prototype, "setItem");
});

describe("apiFetch", () => {
  // Per-test setup is handled by the global beforeEach above to ensure
  // env stubs are set before importing the module and spies persist.
  ...
  describe("URL construction", () => {
    ...
    it("should normalize relative paths to /api/", async () => {
      ...
      console.log("Fetch calls:", vi.mocked(fetch).mock.calls);
  ...
  describe("Error handling", () => {
    ...
    it("should fallback to text response for non-JSON errors", async () => {
      ...
      const resultPromise = apiFetch("employees");
      // Two retries with 2s and 4s backoff
  ...
  describe("Timeout handling", () => {
    it("should return timeout error when request times out", async () => {
      ...
      const resultPromise = apiFetch("employees");
      // First AbortError triggers two retries with backoff 2s and 4s
      await vi.advanceTimersByTimeAsync(100); // trigger first abort
      await vi.advanceTimersByTimeAsync(2000);
      await vi.advanceTimersByTimeAsync(100); // second attempt abort
      await vi.advanceTimersByTimeAsync(4000);
      await vi.advanceTimersByTimeAsync(100); // third attempt abort -> returns
      const result = await resultPromise;
