...
describe("Integration: Frontend ↔ Backend", () => {
  ...
  test("authenticated user can load dashboard with real API calls", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByTestId("performance-management-title")
      ).toBeInTheDocument();
    });

    // Verify manager sees team tab (role-based UI)
    expect(
      screen.getByRole("tab", { name: /team appraisal/i })
    ).toBeInTheDocument();
  });

  test("non-manager user does not see team features", async () => {
    // Get auth token from backend
    const token = await loginAndGetToken();

    // Set up authenticated session for a regular employee (non-manager)
    sessionStorage.setItem("auth_token", token);
    sessionStorage.setItem(
      "auth_user",
      JSON.stringify({
    ...
    // Verify non-manager does NOT see team tab or create button
    expect(screen.queryByRole("tab", { name: /team appraisal/i })).toBeNull();
    expect(
