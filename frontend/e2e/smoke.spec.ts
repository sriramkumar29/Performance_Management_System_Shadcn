...
test.describe("Performance Management System - Smoke Tests", () => {
  test.beforeEach(async ({ page }) => {
    ...
    // Mock GET of created appraisal used by loadAppraisal after saving
    await page.route("**/api/appraisals/123", async (route) => {
  ...
  test("should complete login flow successfully", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /performance management/i })
    ).toBeVisible();
    await expect(
      page.getByText(/welcome back! please sign in to continue/i)
    ).toBeVisible();
  ...
  test("should navigate to protected routes after authentication", async ({
    page,
  }) => {
    // Login first
    await page.goto("/login");
    ...
    await expect(
      page.getByRole("heading", { name: /create new appraisal/i })
    ).toBeVisible();
  ...
  test("should complete create appraisal happy path", async ({ page }) => {
    ...
    // Wait for period to be auto-calculated (current year)
    const year = new Date().getFullYear();
    const expectedStart = `${year}-01-01`;
    const expectedEnd = `${year}-12-31`;
    await expect(page.locator(`input[value="${expectedStart}"]`).first()).toBeVisible();
    await expect(page.locator(`input[value="${expectedEnd}"]`).first()).toBeVisible();
    ...
    await expect(
      page.getByText(/your draft appraisal has been created/i)
    ).toBeVisible({ timeout: 5000 });

    // Status should change to Draft (use badge test id to avoid toast collisions)
    const statusBadge = page.getByTestId("appraisal-status-badge");
  ...
  test("should validate form fields and show appropriate errors", async ({
    ...
    const addGoalButton = page.getByTestId("add-goal-empty");
